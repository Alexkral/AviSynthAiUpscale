...
#define get(x, y) tex2D(s0, float2(x, y)).r
...
float line_run(float ypos, float3 xpos1, float3 xpos2, float3 linetaps1, float3 linetaps2)
{
    return
...
float4 main(float2 tex : TEXCOORD0) : COLOR
{
	...
	line_run(xystart.y + p2.w * 5.0 , xpos1, xpos2, linetaps1, linetaps2) * columntaps2.b, 0, 0, 1);
}
